...
FOLDER_NAME = "data/ofertas_scraped"
...
CARGOS = [
    ...
    "Ingeniero de software", "Ingeniero de datos", "Arquitecto de soluciones", "Arquitecto de software", "Arquitecto de sistemas", 
    "Arquitecto de datos", "Líder de proyectos", "Líder técnico", "Gerente de proyectos", "Gerente de tecnología", "Gerente de desarrollo",
    "Coordinador de sistemas y tecnología", "Coordinador de proyectos"
]

def _normalizar_lista(lista: list) -> list:
    return [item.lower().strip() for item in lista]

def _cargar_historial_postulaciones() -> set:
    ids_historicos = set()
    if not os.path.exists(FOLDER_POSTULACIONES):
        return ids_historicos
        
    for archivo in os.listdir(FOLDER_POSTULACIONES):
        if archivo.endswith(".csv"):
            ruta = os.path.join(FOLDER_POSTULACIONES, archivo)
            try:
...
def _calcular_total_paginas(page: Page) -> int:
    try:
        # Si el selector no existe, lanzará una excepción y pasará al except
        total_texto = page.locator("strong.js-total-results").first.inner_text().strip().replace(".", "")
        total_resultados = int(total_texto)
        
        # Si no hay paginación visible, pero hay resultados, al menos hay 1 página
        if page.locator("select#ResultsByPage").count() == 0 and total_resultados > 0:
            return 1
        ...
        # Si hay tarjetas visibles en el DOM, devolvemos 1 página por defecto
        if page.locator("div.result-item").count() > 0:
            return 1
        return 0
...
def ejecutar_extraccion():
    ...
    ofertas_procesadas = _cargar_historial_postulaciones()
    cantidad_historica = len(ofertas_procesadas)
    ...
    with sync_playwright() as p:
        ...
        for termino in EMPLEOS:
            ...
            paginas_a_procesar = max(1, min(total_paginas, LIMITE_PAGINAS))
    ...
    print(f"IDs históricos omitidos al iniciar: {cantidad_historica}")
    print(f"Nuevas ofertas procesadas hoy: {len(ofertas_procesadas) - cantidad_historica}")
