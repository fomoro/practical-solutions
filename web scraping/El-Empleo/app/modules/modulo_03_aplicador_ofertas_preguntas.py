...
FOLDER_APPROVED = "data/ofertas_scraped"
FOLDER_POSTULACIONES = "data/ofertas_postuladas"
SESSION_FILE = "config/sesion.json"

FOLDER_RESPUESTAS = "data/respuestas" 
JSON_PENDIENTES = "data/preguntas/preguntas_pendientes.json"
TIMEOUT_MS = 3000
...
SELECTOR_BOTON_ESTANDAR = "button.js-apply-offer"
SELECTOR_BOTON_RAPIDO = "button.js-apply-modal-quick-offer"
# NOTA TÉCNICA: Se usa '.hidden-xs' porque el DOM renderiza dos botones idénticos en el modal.
# Uno exclusivo para móviles (visible-xs) y otro para escritorio (hidden-xs). Playwright necesita el de escritorio.
SELECTOR_BOTON_MODAL = "button.js-apply-quick-offer.hidden-xs"
    ...
    if os.path.exists(JSON_PENDIENTES):
        try:
            with open(JSON_PENDIENTES, mode="r", encoding="utf-8") as f:
                ...
                if contenido: # Validación defensiva: Solo parsea si no está vacío
                    historial_pendientes = json.loads(contenido)
        except json.JSONDecodeError:
            print(f"⚠️ Advertencia: El archivo {JSON_PENDIENTES} estaba corrupto o vacío. Se creará uno nuevo.")
        except Exception:
            pass
            
    actualizadas = False
    for preg, datos in nuevas_preguntas.items():
        if preg not in historial_pendientes:
            historial_pendientes[preg] = datos
            actualizadas = True
            
    if actualizadas:
        os.makedirs(os.path.dirname(JSON_PENDIENTES), exist_ok=True)
...
def _cargar_todas_las_respuestas() -> dict:
    banco_unificado = {}
    if os.path.exists(FOLDER_RESPUESTAS):
        for archivo in os.listdir(FOLDER_RESPUESTAS):
            if archivo.endswith(".json"):
                ruta_completa = os.path.join(FOLDER_RESPUESTAS, archivo)
                try:
                    with open(ruta_completa, mode="r", encoding="utf-8") as f:
                        contenido = f.read().strip()
                        if contenido: # Validación defensiva: Solo parsea si no está vacío
                            datos_json = json.loads(contenido)
                            banco_unificado.update(datos_json) 
                except json.JSONDecodeError:
                    print(f"⚠️ Advertencia: El archivo {archivo} está vacío o contiene JSON inválido. Será omitido.")
                except Exception as e:
                    print(f"⚠️ Error al leer {archivo}: {e}")
    else:
        print(f"⚠️ La carpeta {FOLDER_RESPUESTAS} no existe.")
    
    return banco_unificado

def _llenar_cuestionario(page) -> str:
    banco_respuestas = _cargar_todas_las_respuestas()
    ...
    for i in range(preguntas_ui.count()):
        ...
        respuesta_cruda = banco_respuestas.get(texto_limpio)
        
        if respuesta_cruda:
            # Implementación Regla de los Corchetes [ ]
            match = re.match(r'^\[(.*?)\]\s*(.*)', respuesta_cruda)
            if match:
                opcion_corta = match.group(1).strip()
                respuesta_larga = match.group(2).strip()
                if not respuesta_larga:
                    respuesta_larga = opcion_corta
            else:
                opcion_corta = respuesta_cruda.strip()
                respuesta_larga = respuesta_cruda.strip()

            textarea = grupo.locator("textarea")
            if textarea.count() > 0:
                textarea.first.fill(respuesta_larga)
                continue
            ...
            if radios.count() > 0:
                for j in range(radios.count()):
                    ...
                    if opcion_corta.lower() in radio_label.inner_text().strip().lower():
                        radio_label.locator("input[type='radio']").check()
                        break
                continue
        else:
            # Lógica de extracción profunda según el nuevo modelo JSON
            if grupo.locator("textarea").count() > 0:
                preguntas_no_mapeadas[texto_limpio] = {
                    "tipo_control": "Texto Libre"
                }
            elif grupo.locator(".radio label").count() > 0:
                opciones = []
                radios_opts = grupo.locator(".radio label")
                for k in range(radios_opts.count()):
                    texto_opcion = radios_opts.nth(k).inner_text().strip()
                    if texto_opcion:
                        opciones.append(texto_opcion)
                
                preguntas_no_mapeadas[texto_limpio] = {
                    "tipo_control": "Selección Única (Radio)",
                    "opciones_detectadas": opciones
                }
            else:
                preguntas_no_mapeadas[texto_limpio] = {
...
def _aplicar_oferta_estandar(page) -> bool:
    print("🖱️ Flujo Estándar: Haciendo clic para aplicar...")
    page.evaluate(f'document.querySelector("{SELECTOR_BOTON_ESTANDAR}").click()')
    return True

def _aplicar_oferta_rapida(page) -> bool:
    print("🖱️ Flujo Rápido: Abriendo modal de postulación...")
    page.evaluate(f'document.querySelector("{SELECTOR_BOTON_RAPIDO}").click()')
    
    try:
        print("⏳ Esperando botón de confirmación en el modal...")
        boton_modal = page.locator(SELECTOR_BOTON_MODAL).first
        # Espera basada en eventos del DOM (hasta 5 segundos)
        boton_modal.wait_for(state="visible", timeout=5000)
        print("🖱️ Confirmando postulación rápida...")
        # Interacción de bajo nivel como indica el estándar #3
        page.evaluate(f'document.querySelector("{SELECTOR_BOTON_MODAL}").click()')
        return True
    except PlaywrightTimeoutError:
        print("❌ Error: El modal de oferta rápida nunca cargó o el botón no está visible.")
        return False
...
def _procesar_oferta_individual(page, oferta: dict) -> str:
    ...
    try:
        ...
        page.wait_for_timeout(2000) # Pausa inicial de carga de la oferta

        if page.locator(SELECTOR_ALERTA_APLICADO).first.is_visible(timeout=TIMEOUT_MS):
            return "ya_aplicada"

        # Detección y Enrutamiento restaurando tu estándar de .count() > 0
        es_estandar = page.locator(SELECTOR_BOTON_ESTANDAR).count() > 0
        es_rapida = page.locator(SELECTOR_BOTON_RAPIDO).count() > 0

        if not es_estandar and not es_rapida:
        ...
        # Ejecución del camino detectado
        exito = False
        if es_estandar:
            exito = _aplicar_oferta_estandar(page)
        elif es_rapida:
            exito = _aplicar_oferta_rapida(page)
            
        if not exito:
            return "falla_clic_modal"

        # --- CICLO DE VIGILANCIA (Polling Inteligente) ---
        print("⏳ Analizando respuesta de la página...")
        tiempo_esperado = 0
        while tiempo_esperado < 15000:
